    ...
    <nav class="navbar" :style="s.navbar">
      ...
      <div class="nav-links" :style="s.navLinks">
        <a href="https://github.com/nikmcfly/MiroFish-Offline" target="_blank" class="github-link" :style="s.githubLink">
          Besuchen Sie unser Github <span>↗</span>
        </a>
      </div>
    </nav>
      ...
      <section class="hero-section" :style="s.heroSection">
        <div class="hero-left" :style="s.heroLeft">
          <div class="tag-row" :style="s.tagRow">
            <span class="orange-tag" :style="s.orangeTag">Offline Multi-Agent Simulationsmaschine</span>
            <span class="version-text" :style="s.versionText">/ v0.1-preview</span>
          </div>

          <h1 class="main-title" :style="s.mainTitle">
            Laden Sie ein beliebiges Dokument hoch<br>
            <span class="gradient-text" :style="s.gradientText">Sagen Sie vorher, was als Nächstes passiert</span>
          </h1>

          <div class="hero-desc" :style="s.heroDesc">
            <p :style="s.heroDescP">
              Aus einem einzigen Dokument extrahiert <span :style="s.highlightBold">MiroFish Offline</span> Realitätskeime und erstellt eine parallele Welt aus <span :style="s.highlightOrange">autonomen KI-Agenten</span> — vollständig auf Ihrem Rechner. Injizieren Sie Variablen, beobachten Sie emergentes Verhalten und finden Sie <span :style="s.highlightCode">"lokale Optima"</span> in komplexen sozialen Dynamiken.
            </p>
            <p class="slogan-text" :style="s.sloganText">
              Ihre Daten verlassen nie Ihren Rechner. Die Zukunft wird lokal simuliert<span :style="s.blinkingCursor">_</span>
            </p>
          </div>
      ...
      <section class="dashboard-section" :style="s.dashboardSection">
        ...
        <div class="left-panel" :style="s.leftPanel">
          ...
          <p class="section-desc" :style="s.sectionDesc">
            Lokale Vorhersagemaschine im Standby. Laden Sie unstrukturierte Daten hoch, um eine Simulation zu starten.
          </p>
          ...
          <div class="metrics-row" :style="s.metricsRow">
            <div class="metric-card" :style="s.metricCard">
              ...
              <div class="metric-label" :style="s.metricLabel">Läuft auf Ihrer Hardware</div>
            </div>
            <div class="metric-card" :style="s.metricCard">
        ...
        <div class="right-panel" :style="s.rightPanel">
          <div class="console-box" :style="s.consoleBox">
            <div :style="s.consoleSection">
              ...
              >
                ...
                <div v-if="files.length === 0" :style="s.uploadPlaceholder">
                  ...
                  <div :style="s.uploadTitle">Dateien hierher ziehen</div>
            ...
            <div :style="s.consoleSection">
              <div class="console-header" :style="s.consoleHeader">
                <span>>_ 02 / Simulationsanweisung</span>
              </div>
              <div :style="s.inputWrapper">
                <textarea v-model="formData.simulationRequirement" :style="s.codeInput" placeholder="// Beschreiben Sie Ihr Simulations- oder Vorhersageziel in natürlicher Sprache" rows="6" :disabled="loading"></textarea>
            ...
            <div :style="s.btnSection">
              <button :style="s.startEngineBtn" @click="startSimulation" :disabled="!canSubmit || loading">
                <span v-if="!loading">Starten</span>
...
const steps = [
  { num: '01', title: 'Graph erstellen', desc: 'Realitätskeime aus Ihrem Dokument extrahieren, Wissensgraph mit Neo4j + GraphRAG erstellen' },
  { num: '02', title: 'Umgebung einrichten', desc: 'Agent-Personas generieren, Simulationsparameter über lokales Ollama LLM konfigurieren' },
  { num: '03', title: 'Simulation', desc: 'Multi-Agent-Simulation lokal ausführen mit dynamischen Speicheraktualisierungen und emergentem Verhalten' },
  ...
  { num: '05', title: 'Interaktion', desc: 'Chatten Sie mit einem beliebigen Agenten aus der simulierten Welt oder besprechen Sie Ergebnisse mit dem Berichts-Agent' },
]
